...
this code determines the historical behavior of agents in case of different
risk values through r.
"""
...
result_dir = r'C:\Projects\ABM_PGMS\Results\01_Historical_Analysis'                 # results directory
...
#%% Calibration 
"""in this section, random data is generated by gamma, beta, and normal 
distributions to save size, risk, REC, and perceived cost by different agent 
types. For each set of parameters, the code runs for 10 simluations to capture 
the stochasticity of the model. Model is compared for different values of r to
determine historical behavior of agents."""
...
R, KGE = [], []                                                                     # create empty lists to store data
...
for k in range(6):                                                                  # test 10 different values for risk between 0.1 and 1 with an interval of 0.1                                                         
    print('Running simulation for risk={}'.format(risk_values[k]))    
    result = []                                                                     # create a list to store results
    r = risk_values[k]                                                              # set risk to predefined values
    for l in range(10):                                                             # 10 simulations to capture stochasticity
        ...
        dist_cost, dist_RSR = generate_samples(dist_params, result_dir, renewable, 
                                           n_agt,date_now,k)                        # generate two dictionaries containing required random    ng_cost_dist = dist_cost["NG"]                                                  # NG cost
        ...
        for t in range(9):                                                          # zonal demand data only available from 2011 to 2020
            ...
            cap_deficit = cap_predicted - tot_ca + CA.loc[y][1]                     # capacity_deficit = predicted_cpacity - current_capacity + retired_capacit
        ...
        # agg_tech_file = os.path.join(result_dir, date_now, "agg_tech" + str(k) + str(l) +".csv")
        # agg_tech.to_csv(agg_tech_file)                                              # agg_tech: investment by technology types
        agg_year_file = os.path.join(result_dir, date_now, "agg_year" + str(k) + str(l) +".csv")
        agg_year.to_csv(agg_year_file)                                              # agg_year: investment by year
        # agg_LZ_file = os.path.join(result_dir, date_now, "agg_LZ" + str(k) + str(l) +".csv")
        ...
        df_name = str(k) + str(l)                                                   # create a column name based on k and l
...
#%% Plots
time = np.arange(2012,2021,1)                                                       # create time as x axis
df = CP.iloc[-9:,:]                                                                 # extract capacity for 2012-2020 for comparison
fig, axs = plt.subplots(2, 2)                                                       # create four subplots
colors = ['y','c','m','b','r','g']                                                  # create a list of colors
for k in range(6):                                                                  # loop over risk perception values
    for l in range(10):                                                             # loop over stochasticity
        df_name = str(k) + str(l)                                                   # create column name for data
...
fig.tight_layout()
fig.savefig(os.path.join(result_dir, date_now, 'Result' + date_now[-5:] +           # save figure as *.png file
                         '.png'), pad_inches=0, dpi=300)