...
In this code, risk is updated annually by observing temperature data for the 
last year.
"""
...
sys.path.append(r'C:\Projects\ABM_PGMS\ABM')                                        # append ABM directory to path
sys.path.append(r'C:\Projects\ABM_PGMS\PGMS')                                       # append PGMS directory to path                            
# import functions
from ABM_PGMS import pgms2abm, bus2gen, load_average, ZoneInvest, AgentInvest,\
...
#%% read data files☻
# directories
cwd = r'C:\Projects\ABM_PGMS'                                                       # current working directory
os.chdir(cwd)                                                                       # change working directory
result_dir = os.path.join(cwd, 'Results', '03_CC', 'GCAM', '3.0cost_3.0cf')         # result directory
...
temp_data_dir = os.path.join(future_data_dir, "Climate_Scenarios")                  # future temperature data directory

temp_files = [f.path for f in os.scandir(temp_data_dir) if f.path.endswith('.csv')]     # list all *.csv files in future temperature data
for temp_file in temp_files[6:]:                                                            # loop over temperature files
    scen = os.path.basename(temp_file)[:-4]                                             # get the name of scenario
    print('\nStarted simulation for {}\n'.format(scen))
    ...
    Annual_Demand = pd.read_csv(os.path.join(Demand_dir, "Future_annual_demand.csv"),
                                usecols=["Year", "Energy (MWh)"], index_col='Year')     # read ERCTO annual load prediction for 2021-2050
    cost_file = "Cost_3.00_pect_decrease.pkl"                                           # Cost file
    cf_file = "CF_3.00_pect_increase.pkl"                                               # CF file
    ...
    src = os.path.join(cwd, 'Source_GCAM')                                              # source files to run the code
    ...
    bus_load = load_average(os.path.join(cwd,'ABM','Load_Data',
                                         'Loads_bus2gen_GCAM'), 'gcam')               # get average loads for load profiles for each bus
    # parameters
    ...
    REC_p = 10                                                                          # renewable energy credit price ($/MW)
    ...
    #%% future simulation
    df_risk = update_risk(temp_file, n_agt, risk_dist, a0, b0, dst)                     # update risk values based on observed temperature
    ...
    for load_sc in load_scs:                                                            # loop over load scenarios
        print('\nStarted simulation for {}\n'.format(os.path.basename(load_sc)))
        ...
        for t in range(T):                                                              # loop over years
            ...
            ### PGMS
            y = 2021 + t                                                                # year
            risk_dist = np.array(df_risk[y])                                            # select updated risk values for year y
        ...
        # save IRR values
        IRR_list = np.array(IRR_frame).reshape(-1,161).T
        df_IRR = pd.DataFrame(IRR_list)
        df_IRR.to_csv(os.path.join(load_sc, 'IRR_list.csv'), header=None, index=False)
...
print('Execution time:', time.strftime("%H:%M:%S", time.gmtime(elapsed_time)))          # print a message to show elapsed time
