...
cwd = r'C:\Projects\ABM-PDM'                                                       # current working directory
os.chdir(cwd)                                                                       # change working directory
result_dir = os.path.join(cwd, 'Results', '02_SA', 'ERCOT')                         # result 
...
Annual_Demand = pd.read_csv(os.path.join(Demand_dir, "Future_annual_demand.csv"),
                            usecols=["Year", "Energy (MWh)"], index_col='Year')             # read ERCOT annual load prediction for 2021-2051
"""Instead of selecting one combination of future Cost and CF values, there is
a loop over all possible combinations."""
cost_files = [f.path for f in os.scandir(Cost_dir) if f.path.endswith('.pkl')]              # list all *.pkl files in future Cost dir
cf_files = [f.path for f in os.scandir(CF_dir) if f.path.endswith('.pkl')]                  # list all *.pkl files in future CF dir

for cost_file in cost_files:                                                                # loop over Cost files
    for cf_file in cf_files:                                                                # loop over CF files
        scen = 'ERCOT_{}cost_{}cf'.format(cost_file[-22:-18], cf_file[-22:-18])             # get the name of SA scenario
        print('\nStarted simulation for {}\n'.format(scen))
        Cost = pd.read_pickle(cost_file)                                                    # read future Cost data
        CF = pd.read_pickle(cf_file)                                                        # read future CF data
        ...
        #%% future simulation
        # df_risk = update_risk((temp_data_file, n_agt, risk_dist, a0, b0))                   # update risk values based on observed temperature (only active for CC codes)
        ...
        for load_sc in load_scs:                                                            # loop over load scenarios
            ...
            for t in range(T):                                                              # loop over years
                ...
                linear = pgms2abm(pgms_outputs_dir, y, bus_map, LZ, 
                                             linear)                                        # calculate supply curves [slope, intercept] of the load zones (price = a*demand + b)
